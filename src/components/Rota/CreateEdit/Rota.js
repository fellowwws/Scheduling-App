...
function Rota({rota, editing, history}) {
  ...
  const handlePublishOrSaveDraft = () => {
    ...
    if (editing) {
      firestore.collection(state.published ? 'published' : 'unpublished')
        .doc(rota.id)
  ...
  return (
    <Fragment>
      ...
      <Footer>
        <Button
          size="lg"
          block color={state.published ? 'primary' : 'secondary'}
...
function Head({ children }) {
  ...
  return (
    <Row className="pb-2">
      <Col className="pb-2" md="4">{DatePicker}</Col>
      <Col className="pb-1" md="8">{Notes}</Col>
    </Row>
  );
}
